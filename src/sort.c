...
static inline void copyFromArray(char* dest, const char* src,
                                 size_t* mergeIndex, size_t* srcIndex,
                                 size_t entrySizeInBytes) {
    memcpy(dest + *mergeIndex, src + *srcIndex, entrySizeInBytes);
    *srcIndex += entrySizeInBytes;
}

static inline void __CTLCopyRemaining(char* dest, const char* src,
                                      size_t* mergeIndex, size_t* srcIndex,
                                      size_t srcInBytes,
                                      size_t entrySizeInBytes) {
...
static void __CTLMerge(char* array, size_t entrySizeInBytes, size_t left,
    ...
    while (leftIndex < leftArrayByteSize && rightIndex < rightArrayByteSize) {
        if (cmp(leftArray + leftIndex, rightArray + rightIndex) <= 0)
            copyFromArray(array, leftArray, &mergeIndex, &leftIndex,
                          entrySizeInBytes);
        else
            copyFromArray(array, rightArray, &mergeIndex, &rightIndex,
                          entrySizeInBytes);

        mergeIndex += entrySizeInBytes;
    }

    __CTLCopyRemaining(array, leftArray, &mergeIndex, &leftIndex,
                       leftArrayByteSize, entrySizeInBytes);
    __CTLCopyRemaining(array, rightArray, &mergeIndex, &rightIndex,
                       rightArrayByteSize, entrySizeInBytes);
...
static void __CTLMergeSort(void* array, size_t entrySizeInBytes, size_t left,
                           size_t right, CTLCompareFunction cmp) {
    if (left < right) {
